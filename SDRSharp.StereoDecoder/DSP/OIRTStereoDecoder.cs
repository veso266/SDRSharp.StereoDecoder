...
{
    public unsafe class OIRTStereoDecoder : IStereoDecoder
    {
        private const int DefaultPilotFrequency = 31250; //Pilot tone is 31.25 kHz
        private const int PllRange = 20;
        private const int PllBandwith = 10;
        private const float PllThreshold = 2f; //2 is a sweat spot, it locks into this weak polar pilot, but does not lock on normal audio (souch as CCIR that has pilot at 19khz) 
        ...
        //Left,right buffers
        private UnsafeBuffer _channelLeftBuffer;
        private UnsafeBuffer _channelRightBuffer;
        private float* _channelLeftPtr;
        private float* _channelRightPtr;

        private UnsafeBuffer _channelKBuffer;
        private float* _channelKPtr;

        //k_notch
        float[] ad, bd = new float[] { 0f, 0f };



        public OIRTStereoDecoder()
        {
        ...
        private void ProcessStereo(float* baseBand, float* interleavedStereo, int length)
        {
            ...
            #region Prepare Left buffer

            if (_channelLeftBuffer == null || _channelLeftBuffer.Length != (length / _audioDecimationFactor))
            {
                _channelLeftBuffer = UnsafeBuffer.Create((length / _audioDecimationFactor), sizeof(float));
                _channelLeftPtr = (float*)_channelLeftBuffer;
            }

            #endregion

            #region Prepare Right buffer

            if (_channelRightBuffer == null || _channelRightBuffer.Length != (length / _audioDecimationFactor))
            {
                _channelRightBuffer = UnsafeBuffer.Create((length / _audioDecimationFactor), sizeof(float));
                _channelRightPtr = (float*)_channelRightBuffer;
            }
            ...
            for (var i = 0; i < length; i++)
            {
                var pilot = _pilotFilter->Process(baseBand[i]) * 14; //Boost Pilot tone by 14dB since transmitter is reducing it by 14dB
                _pll->Process(pilot);
                _channelBPtr[i] = baseBand[i] * Trig.Sin((float)(_pll->AdjustedPhase));
            }
            ...
            //Here we input out k_notch
            k_notch(_channelBPtr, _channelLeftPtr, true, audioLength);   //L+R
            k_notch(_channelBPtr, _channelRightPtr, false, audioLength); //L-R
            ...
            for (var i = 0; i < audioLength; i++)
            {
                ...
                var notchL = _channelLeftPtr[i];
                var notchR = _channelRightPtr[i];

                interleavedStereo[i * 2] = (a + notchL) * AudioGain;         //Left
                interleavedStereo[i * 2 + 1] = (a + notchR) * AudioGain;     //Right
            }
        ...
        private void ConfigureK_notch()
        {
            ...
            //Assert
            this.ad = DSPUtils.ComplexToReal((Complex[])arlist1[0]);
            this.bd = DSPUtils.ComplexToReal((Complex[])arlist1[1]);
        }

        private void k_notch(float* baseBand, float* filteredSignal, bool plus, int length)
        {
            #region Prepare K filter buffer

            if (_channelKBuffer == null || _channelKBuffer.Length != length)
            {
                _channelKBuffer = UnsafeBuffer.Create(length, sizeof(float));
                _channelKPtr = (float*)_channelKBuffer;
            }

            #endregion

            //filter signal
            DSPUtils.filter(this.ad, this.bd, baseBand, _channelKPtr, length);

            for (int i = 0; i < length; i++)
            {
                if (plus) //If L+R
                    filteredSignal[i] = (_channelKPtr[i]) * 1;
                else //If L-R
                    filteredSignal[i] = (_channelKPtr[i]) * -1;
            }
        }
        ...
        {
            _audioDecimationFactor = (int)Math.Pow(2.0, DecimationRatio); //8 = 3^2??
            ...
            ConfigureK_notch(); //Configure Notch filter
        }
    }
}